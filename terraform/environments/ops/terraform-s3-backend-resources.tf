...
locals {
  root-name = "${var.root-name}"
  region = var.region
}

resource "aws_s3_bucket" "terraform-state" {
    bucket = "${local.root-name}-terraform-state"
...
resource "aws_dynamodb_table" "terraform-locks" {
    name         = "${local.root-name}-terraform-locks"
...
resource "local_file" "backend_file_test" {
  ...
  content = <<-EOT
    terraform {
      backend "s3" {
        bucket = "${local.root-name}-terraform-state"
        key = "${local.root-name}/terraform.tfstate"
        region = "${local.region}"
        dynamodb_table = "${local.root-name}-terraform-locks"
        encrypt = true
      }
    }
  EOT
